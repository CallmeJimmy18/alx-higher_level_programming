...
class Test_base(unittest.TestCase):
    ...
    def test_width_smaller_zero(self):
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
            r_3 = Rectangle(-9, 6, 1, 2, 50)
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
            r_3 = Rectangle(7, 4, 1, 2, 50)
            r_3.width = -6

    def test_height_smaller_zero(self):
        with self.assertRaisesRegex(ValueError, "height must be > 0"):
            r_3 = Rectangle(6, -6, 1, 2, 50)
        with self.assertRaisesRegex(ValueError, "height must be > 0"):
            r_3 = Rectangle(7, 4, 1, 2, 50)
            r_3.height = -2

    def test_x_eqauls_zero(self):
        r_3 = Rectangle(7, 4, 0, 2, 50)
        self.assertEqual(r_3.x, 0)

    def test_y_eqauls_zero(self):
        r_3 = Rectangle(7, 4, 3, 0, 50)
        self.assertEqual(r_3.y, 0)

    def test_x_smaller_than_zero(self):
        with self.assertRaisesRegex(ValueError, "x must be >= 0"):
            r = Rectangle(9, 6, -1, 2, 50)
        with self.assertRaisesRegex(ValueError, "x must be >= 0"):
            r = Rectangle(7, 4, 1, 2, 50)
            r.x = -6

    def test_y_smaller_than_zero(self):
        with self.assertRaisesRegex(ValueError, "y must be >= 0"):
            r = Rectangle(6, 7, 1, -2, 50)
        with self.assertRaisesRegex(ValueError, "y must be >= 0"):
            r = Rectangle(7, 4, 1, 2, 50)
            r.y = -2
    ...
    def test_normal_rectangle_using_string(self):
        r_8 = Rectangle(4, 4, 7, 2, 3)
        self.assertEqual(str(r_8), "[Rectangle] (3) 7/2 - 4/4")
    ...
    def test_print_width_and_height_of_one(self):
        r_8 = Rectangle(1, 1, 0, 0, 0)
        self.assertEqual(str(r_8), "[Rectangle] (0) 0/0 - 1/1")

    def test_print_no_x_y_or_id(self):
        r_8 = Rectangle(1, 1)
        self.assertEqual(str(r_8), "[Rectangle] ({}) 0/0 - 1/1".format(r_8.id))

    def test_some_args(self):
        r_6 = Rectangle(1, 2, 3, 4)
        r_7 = Rectangle(1, 2, 3)
        r_8 = Rectangle(1, 2,)
        self.assertEqual(str(r_6), "[Rectangle] ({}) 3/4 - 1/2".format(r_6.id))
        self.assertEqual(str(r_7), "[Rectangle] ({}) 3/0 - 1/2".format(r_7.id))
        self.assertEqual(str(r_8), "[Rectangle] ({}) 0/0 - 1/2".format(r_8.id))

    def test_normal_update_args(self):
        r = Rectangle(1, 2, 3, 5, 7)
        r.update(6, 7, 8, 2, 10)
        self.assertEqual(str(r), "[Rectangle] (6) 2/10 - 7/8")

    def test_modify_after_update(self):
        r = Rectangle(1, 2, 3, 5, 7)
        r.update(6, 7, 8, 2, 10)
        r.id = 200
        self.assertEqual(str(r), "[Rectangle] (200) 2/10 - 7/8")
...
class test_raises(unittest.TestCase):
    ...
    def test_smallerthan_zero(self):
        r2 = Rectangle(5, 7, 8, 4, 8)
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
    ...
    def test_None(self):
        r = Rectangle(5, 7, 8, 4, 8)
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            r.update(id=6, width=None, height=8, x=8, y=9)
        with self.assertRaisesRegex(TypeError, "height must be an integer"):
            r.update(id=6, width=7, height=None, x=8, y=9)
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            r.update(id=6, width=8, height=8, x=None, y=9)
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            r.update(id=6, width=6, height=8, x=8, y=None)

    def test_dictionary_normal(self):
        r2 = Rectangle(10, 2, 1, 1, 1)
        expect = {'id': 1, 'x': 1, 'width': 10, 'height': 2, 'y': 1}
        self.assertDictEqual(r2.to_dictionary(), expect)

    def test_new_obj_dictionary(self):
        r_1 = Rectangle(10, 2, 1, 1, 1)
        r_2 = Rectangle(10, 2, 1, 1, 1)
        expect = {'id': 1, 'x': 1, 'width': 10, 'height': 2, 'y': 1}
        self.assertDictEqual(r_2.to_dictionary(), expect)

    def test_normal_dictionary(self):
        r2 = Rectangle(10, 2, 1, 1)
        expect = {'id': r2.id, 'x': 1, 'width': 10, 'height': 2, 'y': 1}
        self.assertDictEqual(r2.to_dictionary(), expect)
